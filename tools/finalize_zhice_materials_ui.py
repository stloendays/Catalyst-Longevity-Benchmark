...
text = replace_once(text,
    ...
    'page spacing')
text = replace_once(text, '    sourceFrame->setMaximumHeight(96);\n', '    sourceFrame->setMaximumHeight(82);\n', 'source height')
text = replace_once(text,
...
text = replace_once(text,
    ...
    'packet compact')
text = replace_once(text, '    documentInfoLabel_->setText(QStringLiteral("—"));\n', '    documentInfoLabel_->clear();\n', 'empty document info')
text = replace_once(text,
...
p = Path("native/qt/src/evidencepacket.cpp")
text = p.read_text(encoding="utf-8")
text = text.replace(
...
text = text.replace(
    'QStringLiteral("部分证据缺少可追溯来源标识。")',
    'QStringLiteral("部分资料缺少可追溯来源标识。")')
start = text.index('QString EvidencePacketBuilder::toMarkdown')
end = text.index('\n}\n\n} // namespace catalyst', start) + 2
new_func = r'''QString EvidencePacketBuilder::toMarkdown(const EvidencePacket& packet) {
    QString text;
...
p.write_text(text, encoding="utf-8")
print("final Zhice materials UI migration applied")
