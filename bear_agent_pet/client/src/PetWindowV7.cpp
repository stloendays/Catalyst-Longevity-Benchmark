...
void PetWindow::paintEvent(QPaintEvent*) {
    ...
    if(const QPixmap *sprite=pixmapForAction(action_); sprite && !sprite->isNull()) {
        ...
        // Absolute zero-crop invariant: every non-transparent pixel of Tony must
...
void PetWindow::perchOnForegroundWindow(){
    ...
    if((target-pos()).manhattanLength()>2) {
        move(target);
        bubble_.follow(mapToGlobal(QPoint(width()/2,4)));
        composer_.follow(mapToGlobal(QPoint(width()/2,4)));
    }
